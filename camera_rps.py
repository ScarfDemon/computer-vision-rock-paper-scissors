...
class RPS():
    ...
    def get_user_choice(self, predicted_user_choice, probability): # used to get the user's choice after the countdown for a round
    ...
    def RPS_round(self): # runs a round of RPS: counts down, states game, winner and updates scores
        ...
        def display_text_countdown(text, t): # Used to display coundown text saying Rock, Paper, Scissors, Shoot! at the correct time
            if (t < (time.time() - self.t_init) < (t+1)) and  ((time.time() - self.t_init) < (self.time_per_round - 2)):
                cv2.putText(self.frame, text, (100,400), cv2.FONT_HERSHEY_SIMPLEX ,  2, (192, 192, 192), 2, cv2.LINE_AA)
                cv2.imshow('frame', self.frame)
                cv2.waitKey(2)
        ...
        while True:
            ...
            # between 6-7 seconds, display user's choice vs computer's choice
                ...
                cv2.putText(self.frame, f"{final_user_choice} vs {self.computer_choice}", (50, 300), cv2.FONT_HERSHEY_SIMPLEX ,  3, (192, 192, 192), 2, cv2.LINE_AA)
                cv2.imshow('frame', self.frame)
                cv2.waitKey(2)   
    ...
    def end_game(self): 
        ...
        while (time.time()-t_init) <= 2: 
            ...
            if (time.time()-t_init) > 2:
                break
        self.reset_scores() # reset the scores at the end of the game
